...
public class tls_sigcheck
{
    ...
    /**
     * @param strExpire 超时时长，以秒为单位，建议使用不超过�?个月，如果是�?天，10*24*3600=1116000，将字符�? 1116000 填入即可
     * @param strAppid3rd 如果是自有账号，填写�? SdkAppid �?样的值可以（但一定要填）
     * @param SdkAppid 创建应用时，页面上分配的 sdkappid
     * @param strIdentifier 用户标示符，即用�? id
    ...
    /**
     * 校验sig
     * @param userSig sig 字符�?
     * @param pubKey 公钥内容
     * @param sdkAppid 配置页面上的 sdkappid
     * @param identifier 用户 id
     * @return 0 为成功，非零为失败，使用 getErrMsg() 获取错误信息�?
     * 另外可以通过 getExpireTime() �? getInitTime() 获取有效期和生成时间
     */
