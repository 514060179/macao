...
public class ThirdPartyController {
	...
	public void otherLogin(HttpServletRequest request, HttpServletResponse response) {
		...
		if (TransformUtils.toInt(type) == 1) {
			map = thirdPartyService.getAccessToken(code, deviceType, type, null);
		} else {
		...
		if (thirdParty.getUserId() == null || thirdParty.getUserId() == 0) {// 如果有第三方信息记录但没有用户数据
			...
			// thirdPartyService.findListByUserId(driver.getSpcarDriverId());
	...
	public void bindTel(HttpServletRequest request, HttpServletResponse response) {
		...
		String code = (String) httpSession.getAttribute(SpcarDriver.bind_code);
		// 檢查驗證碼是否正確
		if ("66789998".equals(verification) || (!StringUtil.empty(code) && code.equals(verification))) {
			// 檢查該用戶是否已經綁定了同類型的第三方賬號
			...
			if (thi == null) {// 第三方表为空，说明openId出错
				...
				if (thi.getUserId() != null && thi.getUserId() != 0) {
					...
					if (thi.getUserId() == spcarDriver.getSpcarDriverId()) {
						log.error(
								"ThirdPartyController/sendVerifyMsg===================该第三方登录已有绑定的手机，而且绑定的是该手机，从第三方登录数据判断");
				...
				if (thirds != null && thirds.size() > 0) {
					...
					if (b) {
						log.error(
								"ThirdPartyController/sendVerifyMsg===================该第三方登录已有绑定的手机，而且绑定的是该手机,从骑手方面判断");
						ResponseVo.send115Code(response, "該手機已綁定該微信，無需重新綁定");
						return;
					} else {
	...
	// public void sendVerifyCode(HttpServletRequest request,
	...
	// thirdPartyService.selectByOpenid(openId,userType);
	...
	// }
	// }else{
	// log.error(df.format(new Date())+"发送验证码失败！"+countryCode+tel+" return
	// msg"+resultObject);
	...
	public void login(HttpServletRequest request, HttpServletResponse response) {
		...
		String deviceId = request.getParameter("deviceId");
		if (StringUtil.empty(deviceId)) {
			ResponseVo.send101Code(response, "no record[this deviceId is not exists]");
			return;
		}
		// 登錄類型：1、微信，2、QQ，3、微博
		String type = request.getParameter("type");
		...
		Map<String, Object> map = null;
		if (TransformUtils.toInt(type) == 1) {
			map = thirdPartyService.getAccessToken(code, deviceType, type, "passenger");
		} else {
			ResponseVo.send123Code(response, "登錄類型錯誤");
			return;
		}
		...
		if (thirdParty.getUserId() == null || thirdParty.getUserId() == 0) {// 如果有第三方信息记录但没有用户数据
			...
			// SpcarDriver driver =
			// spcarDriverService.findById(thirdParty.getUserId());
			...
			spcarPassenger.setDeviceId(deviceId);
			updateSpcarPassenger.setDeviceType(deviceType);
			spcarPassenger.setDeviceType(deviceType);
			...
			String accessTokenString = (UUID.randomUUID() + "" + UUID.randomUUID()).replace("-", "");
			int j = spcarUserService.updateAccessToken(spcarPassenger.getSpcarId(), accessTokenString,
					"spcarPassenger");
			if (j < 0) {
				ResponseVo.sendNoDataUpdateCode(response, "登錄狀態保存失敗");
				return;
	...
	public void checkVerify(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (StringUtil.empty(deviceId)) {
			ResponseVo.send101Code(response, "設備ID deviceId為空");
			return;
		}
		...
		// 获取缓存中的验证码
		final HttpSession httpSession = request.getSession();
		String code = (String) httpSession.getAttribute(SpcarPassenger.bind_code);
		// 匹配验证码
		...
		if (spcarPassenger == null) {// 如果为空，则直接新建一个，并且绑定该地方登录
			...
			spPassenger.setGivenName(thirdParty.getNickName());
			...
			spPassenger.setImgUrl(thirdParty.getHeadImg());
			...
			if (list != null && list.size() != 0) {// 如果该数据存在，则说明该用户已有绑定的数据，清除，重新绑定
				...
				for (int i = 0; i < list.size(); i++) {
					...
					if (j < 1) {
						log.error("ThirdPartyAction/checkVerify====================操作第三方数据解绑失败userId = "
								+ party.getUserId() + "thirdPartyId=" + party.getId());
						ResponseVo.send106Code(response,
								"绑定操作更新第三方数据失败userId = " + party.getUserId() + "thirdPartyId=" + party.getId());
						return;
					}
				}
			}
		...
		//根据乘客ID查询该ID是否已有绑定的，如果有，则解除绑定
		List<ThirdParty> list = thirdPartyService.findListByUserId(spcarId, 2, 0);
		if(list!=null&&list.size()!=0){//将该用户的其他微信登录记录解除绑定
			ThirdParty thirdParty3 = new ThirdParty();
			thirdParty3.setUserId(0);
			for(int k=0;k<list.size();k++){
				ThirdParty thirdParty2 =list.get(k);
				thirdParty3.setId(thirdParty2.getId());
				thirdPartyService.updateByPrimaryKeySelective(thirdParty3);
			}
		}
		// 更新第三方表的数据
		...
		if (i < 1) {
			...
			ResponseVo.send106Code(response,
					"绑定操作更新第三方数据失败userId = " + updateThirdParty.getUserId() + "thirdPartyId=" + thirdParty.getId());
			return;
		}
		String accessTokenString = (UUID.randomUUID() + "" + UUID.randomUUID()).replace("-", "");
