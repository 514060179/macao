...
function addMsg(msg,prepend) {
    ...
    //昵称  消息时间
    //var html = webim.Tool.formatText2Html(fromAccountNick + "&nbsp;&nbsp;" + webim.Tool.formatTimeStamp(msg.getTime()));
    //msghead.innerHTML = webim.Tool.formatText2Html(fromAccountNick + "用戶&nbsp;&nbsp;" + webim.Tool.formatTimeStamp(msg.getTime()));
    var tag_list = [
    ...
    webim.getProfilePortrait(
            ...
            function (resp) {
            	if (resp.UserProfileItem && resp.UserProfileItem.length > 0) {
            		if(resp.UserProfileItem[0].ProfileItem){
            			fromAccountNick = resp.UserProfileItem[0].ProfileItem[0].Value;
            		}
            		//console.log(resp);
...
//                html = webim.Tool.formatText2Html(html);
            	/*此处的修改，注释了上面两句，因为不能将文本与表情图片等一起调用formatText2Html()方法格式化，否则会将表情图片的标签等转换成文本格式而不能正确显示这些图片*/
            	var content_temp = convertTextMsgToHtml(content);
            	html += webim.Tool.formatText2Html(content_temp);
